...
const deleteBookingsByStatus = async (req, res) => {
  ...
  try {
    ...
    const bookingsToDelete = await ArtistBooking.find({ paymentStatus: status.toLowerCase() });
    ...
    const result = await ArtistBooking.deleteMany({ paymentStatus: status.toLowerCase() });
